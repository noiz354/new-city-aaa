...
export class Growth {
  ...
  onDay(tick: number): GrowthResult {
    let spawned = 0;
    let movedIn = 0;
    // 1) Move-in: occupy completed buildings still at zero occupants.
    this.buildings.forEachLive((b) => {
      if (b.state === BUILDING_OCCUPIED && b.occupants === 0) {
        const cap = BUILDING_CAPACITY[b.zone]?.[b.level] ?? 0;
        if (cap > 0 && this.buildings.setOccupants(b.id, cap)) movedIn++;
      }
    });
    // 2) Spawn: score every eligible lot, take the top-N (N = pacing cap).
    const w = this.world;
    ...
    for (let y = 0; y < w.size; y++) {
      for (let x = 0; x < w.size; x++) {
        ...
        if (demand <= 0 || !this.hasRoadAccess(x, y)) continue;
  ...
  /** v0 road access: any 4-neighbour is road (T-204 replaces with path-based access). */
  private hasRoadAccess(x: number, y: number): boolean {
    const w = this.world;
    return (
      (x > 0 && (w.road[w.idx(x - 1, y)] as number) === 1) ||
      (x < w.size - 1 && (w.road[w.idx(x + 1, y)] as number) === 1) ||
      (y > 0 && (w.road[w.idx(x, y - 1)] as number) === 1) ||
      (y < w.size - 1 && (w.road[w.idx(x, y + 1)] as number) === 1)
    );
  }
  ...
  growthBlockReason(x: number, y: number): 'unzoned' | 'occupied' | 'no-demand' | 'no-road-access' | null {
    ...
    if (!this.hasRoadAccess(x, y)) return 'no-road-access';
    return null;
  }
